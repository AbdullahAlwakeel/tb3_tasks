...
def extract_secret_from_rows(rows, w, ct):
    channels = {2: 3, 6: 4}[ct]
    # Channel byte order as stored in PNG: RGB or RGBA.
    if ct == 2:
        channel_orders = [
            (0,), (1,), (2,),
            (0,1,2),
    ...
    max_bits = 4096
    # Decode/regex search only over first ~256 decoded bytes per mode (enough for 16-byte token).
    max_search_bytes = 256

    for bitplane in range(8):
        for ch_order in channel_orders:
            # Precompute the required number of channel-bytes read to reach max_bits.
            # Each pixel contributes len(ch_order) bits.
            bits_per_pixel = len(ch_order)
            # We'll traverse pixels row-major until we collect max_bits.
            bits = []
            # rows are full-width for first max_rows; we only need first few pixels.
            for row in rows:
                # number of pixels available in this row
            ...
            # For each possible bit-offset within the first byte.
            for shift in range(8):
                # Decode bytes from bits[shift:] using both bit packing conventions.
                for msb_mode in (0, 1):
                    decoded = bytearray()
                    bit_i = shift
                    while bit_i + 7 < len(bits) and len(decoded) < max_search_bytes:
                        ...
                        if msb_mode == 0:
                            # lsb_first
                            val = (b0[0] << 0) | (b0[1] << 1) | (b0[2] << 2) | (b0[3] << 3) | (b0[4] << 4) | (b0[5] << 5) | (b0[6] << 6) | (b0[7] << 7)
                        else:
                            # msb_first
                            val = (b0[0] << 7) | (b0[1] << 6) | (b0[2] << 5) | (b0[3] << 4) | (b0[4] << 3) | (b0[5] << 2) | (b0[6] << 1) | (b0[7] << 0)
                        decoded.append(val)
                        bit_i += 8
...
def main():
    ...
    for p in sorted(glob.glob('puzzle_*.png')):
        ...
        # Need enough pixels to provide ~max_bits single-channel bits.
        pixel_needed = 4096
        max_rows = min(h, (pixel_needed + w - 1)//w + 2)
        rows, ww, ctt = decode_first_rows(p, max_rows)
        assert ww == w and ctt == ct

        sec = extract_secret_from_rows(rows, w, ct)
        out_lines.append(f"{base}\t{sec if sec else '<not found>'}")
        print(base, sec)

    with open('working_temp/secrets_extracted_v3.txt','w') as f:
        f.write('\n'.join(out_lines)+'\n')
