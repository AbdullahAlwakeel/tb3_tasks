...
def search_secret_in_rows(rows, w, ct, pixel_budget=10000):
    channels = {2: 3, 6: 4}[ct]
    # Build channel orders to try.
    if ct == 2:
        channel_orders = [ (0,), (1,), (2,), (0,1,2), (2,1,0) ]
    else:
        channel_orders = [ (0,), (1,), (2,), (3,), (0,1,2), (2,1,0), (0,1,2,3), (3,2,1,0) ]

    # Flatten first pixel_budget pixels only.
    out = None
    for ch_order in channel_orders:
        ...
        for row in rows:
            ...
            for x in range(n_pix):
                if pixels_used >= pixel_budget:
                    break
                base = x*channels
                for ch in ch_order:
                    stream.append(row[base+ch])
                pixels_used += 1
            if pixels_used >= pixel_budget:
                break

        s = stream.decode('latin1', errors='ignore')
...
def main():
    ...
    for p in sorted(glob.glob('puzzle_*.png')):
        ...
        max_rows = min(h, (10000 + w - 1)//w + 2)
        rows, ww, ctt = decode_first_rows(p, max_rows)
        assert ww==w and ctt==ct
        sec = search_secret_in_rows(rows, w, ct, pixel_budget=10000)
        print(base, sec)
        out.append(f"{base}\t{sec if sec else '<not found>'}")

    with open('working_temp/secrets_extracted_v9.txt','w') as f:
        f.write('\n'.join(out)+'\n')
