import os, re, glob, struct, zlib, math
...
def try_modes(rows, w, ct, pixel_needed=8200, max_search_bytes=1024):
    if ct == 2:
        channel_orders = [ (0,), (1,), (2,), (0,1,2), (2,1,0) ]
    else:
        channel_orders = [ (0,), (1,), (2,), (3,), (0,1,2), (2,1,0), (0,1,2,3), (3,2,1,0) ]

    max_bits = max_search_bytes*8 + 7  # allow shifts
        ...
        for ch_order in channel_orders:
            ...
            for shift in range(8):
                ...
                for bi in range(max_bytes):
                    ...
                    v_lsb = 0
                    v_msb = 0
                    # pack bits[ b_start + 0..7 ] into bytes
                    # lsb_first => first extracted bit goes to bit0
                    # msb_first => first extracted bit goes to bit7
                    for k in range(8):
                ...
                s = decoded_lsb.decode('latin1', errors='ignore')
                m = SECRET_RE.search(s)
                if m:
                    return m.group(0)
                s = decoded_msb.decode('latin1', errors='ignore')
                m = SECRET_RE.search(s)
                if m:
                    return m.group(0)
...
def main():
    ...
    for p in sorted(glob.glob('puzzle_*.png')):
        ...
        # Decode enough full rows for up to pixel_needed pixels row-major.
        max_rows = min(h, (8200 + w - 1)//w + 2)
        rows, ww, ctt = decode_first_rows(p, max_rows)
        assert ww==w and ctt==ct
        sec = try_modes(rows, w, ct, pixel_needed=8200, max_search_bytes=1024)
        print(base, sec)
        out.append(f"{base}\t{sec if sec else '<not found>'}")

    with open('working_temp/secrets_extracted_v6.txt','w') as f:
        f.write('\n'.join(out)+'\n')
