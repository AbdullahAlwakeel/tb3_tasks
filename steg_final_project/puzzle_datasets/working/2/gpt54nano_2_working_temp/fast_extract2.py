...
def decode_png_prefix(path: str, pixels_limit: int = 2048) -> Tuple[int, int, int, bytes]:
    with open(path, 'rb') as f:
        ...
        rows_needed = max(1, min(rows_needed, height))
...
def try_lsb_bytes(raw_prefix: bytes, channels: int, mask: Tuple[int, ...], bit_plane: int, byte_order: str, max_bytes: int = 256) -> Optional[str]:
    ...
    # Precompute bit values for each channel included in mask, for this bit plane.
    bits_by_ch = {}
    for ch in set(mask):
        arr = bytearray(pixel_cap)
        for i in range(pixel_cap):
            arr[i] = (raw_prefix[i * channels + ch] >> bit_plane) & 1
        bits_by_ch[ch] = arr
...
def lsb_extract_secret(path: str, pixels_limit: int = 2048, max_bytes: int = 256) -> Optional[str]:
    _w, _rows, channels, raw_prefix = decode_png_prefix(path, pixels_limit=pixels_limit)
    ...
    for i in all_ch:
        for j in all_ch:
            if i != j:
                masks.append((i, j))
    # all-channel permutations
    if channels <= 5:
        for perm in permutations(all_ch):
            masks.append(perm)
...
def main():
    ...
    out_path = os.path.join('working_temp', 'fast2_secrets.txt')
